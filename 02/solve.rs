...
fn parse_file( path : &str ) -> io::Result<Vec<Vec<i32>>> {
    Ok(aoc::file_lines( path )?.map(|line| line.split_whitespace()
        .map(|token| token.parse::<i32>().expect("not an int?")).collect()).collect())
}
...
fn accept<I>( iter : I ) -> bool where I : Iterator<Item = i32> + Clone {
...
where I : Iterator<Item = i32> + 'a + Clone
{
...
fn main() {
    ...
    println!("part1: {}", data.iter().filter(|&row| accept(row.iter().map(|v|*v))).count());
    println!("part2: {}", data.iter().filter(|&row| (0..row.len()+1).any(
                |ignore| accept(remove_item(row.iter().map(|v|*v), ignore)))).count());
}
